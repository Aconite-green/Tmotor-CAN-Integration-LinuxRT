...
#define TIMEOUT_VALUE_USEC 10000
#define BUFFER_SIZE 256 * 1000

void check(int result, const char* errMsg, int errCode);
int create_socket(const char* ifname);
int configure_socket(int socket, int enable_block, struct timeval tv, struct can_filter rfilter[4], int loopback, int recv_own_msgs);
